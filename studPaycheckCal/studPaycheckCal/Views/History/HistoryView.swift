...
struct HistoryView: View {
    ...
    var body: some View {
        ...
        .onAppear{
            studentPaycheckCoreDataVM.fetchStudentPayCheck()
        }
    }
}
...
struct historySalaryAfterTax: View {
    ...
    @State var currentTab: String = String(Calendar.current.component(.year, from: Date()))
    ...
    var body: some View{
        VStack{
            ...
            VStack(alignment: .leading, spacing: 0) {
                let uniqueYears = Array(Set(studentPaycheckCoreDataVM.studentPayCheckCoreData.map { Calendar.current.component(.year, from: $0.unwrappedDate) }))
                ...
                let totalSalary = studentPaycheckCoreDataVM.studentPayCheckCoreData.reduce(0.0) { result, item in
                    item.salaryAfterTax + result
                }
                
                // Group by Date
                let groupedData = Dictionary(grouping: studentPaycheckCoreDataVM.studentPayCheckCoreData) { paycheckData -> Date in
                    let components = Calendar.current.dateComponents([.year, .month], from: paycheckData.unwrappedDate)
    ...
    var body: some View{
        VStack{
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                // Get the Total Salary Amount for Entire Year
                let totalFedTax = studentPaycheckCoreDataVM.studentPayCheckCoreData.reduce(0.0) { result, item in
                    item.federalTax + result
                }
                
                // Group by Date
                let groupedData = Dictionary(grouping: studentPaycheckCoreDataVM.studentPayCheckCoreData) { historyData -> Date in
